...
import { AlertTriangle, ShieldCheck, FileText, Upload, RefreshCw, CheckCircle, Lock } from "lucide-react";
...
const DEMO_DATA: MatrixItem[] = [
  ...
  { date: "2024-03-15 09:20", event: "Emergency Department Admission", hospitalRecord: "Triaged as Priority 3 (non-urgent). Soft tissue cervical strain noted. Prescribed Mybulen (ibuprofen+paracetamol). Advised to rest 3-5 days.", expertA: "(Orthopaedic Report p.4) 'Patient presented with severe cervical spine pain. Immediate imaging warranted. Missed C5-C6 disc herniation evident on later MRI.'", expertB: "—", riskLevel: "HIGH", strategicAlert: "ALERT: Hospital diagnosis of 'soft tissue strain' contradicts later finding of C5-C6 disc herniation. This is a common RAF defense tactic — arguing the injury is not accident-related but degenerative. Gather evidence of immediate post-accident complaint pattern. Obtain hospital triage nurse statement.", sourcePage: "Hospital p.2, Ortho p.4" },
  { date: "2024-03-15 14:00", event: "Discharge from Hospital", hospitalRecord: "Patient discharged. Ambulatory. Advised to follow up with GP in 7 days if symptoms persist. No follow-up appointment made.", expertA: "(Orthopaedic Report p.5) 'Patient reports being unable to walk unaided for 2 weeks post-accident. Relied on family members for basic mobility.'", expertB: "—", riskLevel: "MEDIUM", strategicAlert: "Gap alert: No medical attendance between discharge (Day 1) and first orthopaedic consult. This 14-day gap will be exploited by RAF to argue insufficient injury severity. Obtain affidavit from family members documenting daily assistance provided. Gather pharmacy records for pain medication purchases.", sourcePage: "Hospital p.4, Ortho p.5" },
  { date: "2024-04-01", event: "Return to Work Attempt", hospitalRecord: "(Patient self-report via RAF1 form) 'Returned to light duties on April 1. Full duties by April 15.'", expertA: "—", expertB: "(Industrial Psy. Report p.10) 'Claimant has been completely incapacitated since the accident. Has not worked a single day. Previous delivery driver role now medically impossible.'", riskLevel: "HIGH", strategicAlert: "FATAL CONTRADICTION: Claimant self-reported returning to work on RAF1 form. Industrial psychologist claims total incapacity. This will destroy the loss of earnings claim if not addressed. Interview claimant immediately. Review bank deposits / payslips for April 2024. Amend RAF1 if clerical error.", sourcePage: "RAF1 form p.3, Ind Psych p.10" },
  { date: "2024-06-10", event: "Orthopaedic Surgery Scheduled", hospitalRecord: "—", expertA: "(Orthopaedic Report p.6) 'C5-C6 anterior cervical discectomy and fusion recommended. Estimated cost: R185,000 at private facility. 6-12 month recovery.'", expertB: "(Industrial Psy. Report p.12) 'Post-surgical recovery will extend functional incapacity to minimum 18 months from accident date. Retraining as sedentary worker required.'", riskLevel: "MEDIUM", strategicAlert: "Quantum impact: Surgery cost + 18-month loss of earnings creates substantial general damages claim. Ensure updated RAF4 Serious Injury Assessment is obtained post-surgery for the narrative test gate. Obtain private hospital quote on letterhead.", sourcePage: "Ortho p.6, Ind Psych p.12" },
];
        ...
        {step === 1 && (
          <form onSubmit={executeLiveAnalysis} className="space-y-8 bg-white p-8 rounded-2xl border border-slate-200 shadow-sm">
            <div className="flex items-start justify-between">
              ...
              <button type="button" onClick={loadDemoCase} className="text-xs text-amber-600 hover:text-amber-800 font-bold border border-amber-300 rounded-lg px-3 py-1.5 hover:bg-amber-50 transition-all">
                ← Try Demo Case First
              </button>
            </div>
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 text-xs text-slate-600 leading-relaxed">
              <div className="flex items-start gap-2 mb-3">
                ...
                <div>
                  ...
                  <p>By uploading documents, you confirm: (1) You are the responsible party as defined in POPIA; (2) You have client consent or lawful basis to process this data; (3) ChronoLegal acts as your operator per Section 21. Files are processed in Johannesburg, South Africa, encrypted with AES-256, and permanently deleted within 24 hours. Only the derived, anonymized chronological matrix is retained. <a href="#" className="text-amber-600 underline">View full Operator Agreement →</a></p>
                </div>
              </div>
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" checked={consented} onChange={e=>setConsented(e.target.checked)} className="rounded border-slate-300 text-amber-500 focus:ring-amber-500"/>
                <span className="font-medium">I confirm client consent and accept the data processing terms above.</span>
              </label>
            </div>
            ...
            <div className="pt-6 border-t flex justify-between items-center">
              ...
              <button type="submit" disabled={loading || !allFilesLoaded || !consented} className="bg-slate-950 text-white font-bold px-8 py-3 rounded-xl flex items-center gap-3 shadow hover:bg-slate-900 transition-all disabled:opacity-40 disabled:cursor-not-allowed">
                {loading ? <><RefreshCw className="h-5 w-5 animate-spin text-amber-400"/> Analyse Trial Bundle...</> : "Execute Discrepancy Engine"}
              </button>
            </div>
            ...
            {loading && processingStage && (
              <div className="text-center py-4">
                <p className="text-sm text-slate-500 font-mono animate-pulse">{processingStage}</p>
              </div>
            )}
          </form>
        )}
          ...
          <div className="space-y-6">
            <div className="bg-white border p-6 rounded-2xl shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-1 font-mono">Reference: {caseRef} · Accident Date: {accidentDate}{isDemo ? " · Sample Data" : ""}</p>
              </div>
                              ...
                              <td colSpan={6} className="px-4 pb-4 pt-0">
                                <div className={`p-4 rounded-xl text-xs flex items-start gap-3 border ${isHigh?"border-red-200 bg-white text-red-900":"border-amber-200 bg-white text-amber-900"}`}>
                                  ...
                                  <div className="flex-1">
                                    ...
                                    {item.sourcePage && (
                                      <div className="text-[10px] font-mono text-slate-400 border-t border-slate-200 pt-2 mt-2">
